-- =====================================================
-- SCRIPT PARA SISTEMA DE PUNTOS (VERSIÓN CORREGIDA)
-- =====================================================
...
CREATE TABLE IF NOT EXISTS asignaciones_puntos (
    id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
...
CREATE INDEX IF NOT EXISTS idx_sistema_puntos_dueno_id ON sistema_puntos(dueno_id);
CREATE INDEX IF NOT EXISTS idx_sistema_puntos_activo ON sistema_puntos(activo);
CREATE INDEX IF NOT EXISTS idx_asignaciones_puntos_sistema_id ON asignaciones_puntos(sistema_puntos_id);
CREATE INDEX IF NOT EXISTS idx_asignaciones_puntos_admin_id ON asignaciones_puntos(admin_id);
CREATE INDEX IF NOT EXISTS idx_notificaciones_usuario_id ON notificaciones_sistema(usuario_id);
CREATE INDEX IF NOT EXISTS idx_notificaciones_leida ON notificaciones_sistema(leida);

-- 5. Función para actualizar updated_at
CREATE OR REPLACE FUNCTION update_sistema_puntos_updated_at()
RETURNS TRIGGER AS $$
BEGIN
    NEW.updated_at = NOW();
    RETURN NEW;
...
CREATE OR REPLACE FUNCTION consumir_puntos_pedido(
    p_dueno_id UUID,
...
CREATE OR REPLACE FUNCTION asignar_puntos_dueno(
    p_dueno_id UUID,
    p_puntos INTEGER,
...
CREATE OR REPLACE FUNCTION verificar_estado_restaurantes_por_puntos()
RETURNS void AS $$
BEGIN
...
-- 11. Políticas RLS para sistema_puntos (con manejo de duplicados)
...
-- 12. Políticas RLS para asignaciones_puntos (con manejo de duplicados)
...
-- 13. Políticas RLS para notificaciones_sistema (con manejo de duplicados)
...
-- 14. Vista para dashboard de puntos
CREATE OR REPLACE VIEW dashboard_puntos AS
SELECT 
    sp.id,
    sp.dueno_id,
...
-- 15. Función para obtener estadísticas de puntos
CREATE OR REPLACE FUNCTION obtener_estadisticas_puntos()
RETURNS TABLE (
    total_duenos BIGINT,
